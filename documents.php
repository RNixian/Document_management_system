...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['folder_name'])) {
    ...
    $division_id = $_POST['division_id'] ?? null; // from dropdown if user selects

    if ($folderName !== '') {
        try {
            // ✅ If no division_id is passed, auto-fetch the first one
...
// ================== Logged-in user’s Divisions ==================
$userDivisions   = [];
$userDivisionIds = [];

try {
    // Fetch both id + name for dropdown
    $stmt = $db->prepare("
        SELECT d.id, d.name
    ...
    $userDivisionIds = array_column($userDivisions, 'id'); // extract IDs
} catch (PDOException $e) {
    $userDivisions   = [];
    $userDivisionIds = [];
}
...
if (!empty($userDivisionIds)) {
    ...
    // 🔹 Only show:
    //    - Docs from same division AND public
    //    - OR docs uploaded by this user (private or public)
    $where[] = "(
